...
export async function getNearestEmbeddings(
    ...
    for (const e of result.rows) {
        ...
        log.debug(`Adding before and after chunks for entry: ${JSON.stringify({ ...e, text: "" })}`);
        ...
        `);
        log.debug(`Found ${entries.rows.length} additional chunks for knowledgeEntryId: ${e.knowledgeEntryId}`);
