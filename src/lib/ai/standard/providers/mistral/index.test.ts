...
if (!apiKey) {
  console.warn(
    "⚠️ MISTRAL_API_KEY not found in environment variables. Tests will be skipped."
  );
}
...
describe("Mistral Provider", () => {
  ...
  conditionalTest(
    "generateEmbedding should handle API errors gracefully",
    async () => {
      const text = "Hello, world!";
      try {
        const response = await mistralProvider.generateEmbedding(text);
        // If we reach here, the test should fail
