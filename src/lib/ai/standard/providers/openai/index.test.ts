...
if (!apiKey) {
  console.warn(
    "⚠️ OPENAI_API_KEY not found in environment variables. Tests will be skipped."
  );
}
...
describe("OpenAI Provider", () => {
  ...
  conditionalTest(
    ...
    async () => {
      ...
      const response = await openaiProvider.generateText(messages);
  ...
  conditionalTest(
    ...
    async () => {
      ...
      const response = await openaiProvider.generateLongText(messages, {
        desiredWords,
      });
  ...
  conditionalTest(
    ...
    async () => {
      ...
      const invalidProvider = new OpenAIProvider("invalid-api-key");
