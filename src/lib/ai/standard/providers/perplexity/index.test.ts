...
if (!apiKey) {
  console.warn(
    "⚠️ PERPLEXITY_API_KEY not found in environment variables. Tests will be skipped."
  );
}

// Create a provider instance for testing
const perplexityProvider = new PerplexityProvider(apiKey || "");
...
describe("Perplexity Provider", () => {
  ...
  test("Constructor should initialize with API key and default base URL", () => {
    const provider = new PerplexityProvider("test-api-key");
  ...
  //         role: "user",
  //         content:
  //           "Return a JSON object with the following structure: { name: 'Paris', country: 'France', population: 2161000 }. You will answer only and directly with the JSON object, no other text or commentary.",
  //       },
  //     ];
  ...
  conditionalTest(
    ...
    async () => {
      ...
      const response = await perplexityProvider.generateLongText(messages, {
        desiredWords,
      });
  ...
  conditionalTest(
    ...
    async () => {
      ...
      const invalidProvider = new PerplexityProvider("invalid-api-key");
