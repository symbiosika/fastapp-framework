...
import { InputGuardrail, InputGuardrailResult, OutputGuardrail, OutputGuardrailResult } from "./types";
...
export function createInputGuardrail(
  checkFn: (input: string | ChatMessage[], context: ChatSessionContext) => Promise<InputGuardrailResult> | InputGuardrailResult,
  options: {
    name?: string;
    description?: string;
  } = {}
  ...
  return {
    ...
    check: async (input: string | ChatMessage[], context: ChatSessionContext) => {
...
export function createOutputGuardrail(
  checkFn: (output: any, context: ChatSessionContext) => Promise<OutputGuardrailResult> | OutputGuardrailResult,
  options: {
    name?: string;
    description?: string;
  } = {}
...
export function createPiiDetectionGuardrail(): InputGuardrail {
  return createInputGuardrail(
    async (input: string | ChatMessage[], context: ChatSessionContext) => {
      ...
      return {
        allowed: !hasPii,
        reason: hasPii ? 'Content contains personally identifiable information' : undefined
      };
    },
    {
      name: 'pii_detection',
