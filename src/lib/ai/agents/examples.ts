...
export function createAgentWithTools() {
  ...
  const agent = new Agent({
    name: "WeatherAgent",
    instructions:
      "You are a weather assistant. Help users get weather information for different cities.",
    model: "gpt-4",
...
export function createAgentsWithHandoffs() {
  ...
  const mainAgent = new Agent({
    name: "MainAssistant",
    instructions:
      "You are a helpful assistant. If users ask about weather, hand off to the weather agent.",
    model: "gpt-4",
    handoffs: [
      handoff(weatherAgent, {
        name: "weather_handoff",
        description:
          "Hand off to the weather agent for weather-related queries",
      }),
    ],
  });
...
export async function createMultiAgentWorkflow() {
  ...
  const translationAgent = new Agent({
    ...
    instructions:
      "You are a translation assistant. Translate text between languages accurately.",
    model: "gpt-4",
    tools: [
      functionTool(
        async (
  ...
  const summarizationAgent = new Agent({
    ...
    instructions:
      "You are a summarization assistant. Create concise summaries of text.",
    model: "gpt-4",
    tools: [
      functionTool(
        async (
          args: { text: string; maxLength?: number },
          context: ChatSessionContext
        ) => {
  ...
  const formattingAgent = new Agent({
    ...
    instructions:
      "You are a formatting assistant. Format text in a structured way.",
    model: "gpt-4",
    tools: [
      functionTool(
        async (
        ...
        },
        {
          ...
          parameters: {
            ...
            format: {
              type: "string",
              description:
                "The format to apply (e.g., 'markdown', 'html', 'json')",
            },
          },
        }
      ),
  ...
  const result = await executeAgentWorkflow(
    ...
    context,
    {
      ...
      onAgentStart: (agent, input) => {
        console.log(
          `Starting agent: ${agent.name} with input: ${typeof input === "string" ? input.substring(0, 30) + "..." : "complex input"}`
        );
      },
      onAgentEnd: (agent, result) => {
        console.log(
          `Agent ${agent.name} completed with output: ${JSON.stringify(result.output).substring(0, 50)}...`
        );
      },
    }
  );
