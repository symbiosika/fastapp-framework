...
export function definePublicUserRoutes(
   ...
   * Login endpoint
   */
  app.post(API_BASE_PATH + "/user/login", async (c: Context) => {
    try {
   ...
   */
  app.get(API_BASE_PATH + "/user/send-magic-link", async (c: Context) => {
   ...
   */
  app.get(API_BASE_PATH + "/user/verify-email", async (c: Context) => {
   ...
   * Register endpoint
   */
  app.post(API_BASE_PATH + "/user/register", async (c: Context) => {
    try {
      if (_GLOBAL_SERVER_CONFIG.authType !== "local") {
        throw new HTTPException(400, {
   ...
   */
  app.post(API_BASE_PATH + "/user/forgot-password", async (c: Context) => {
    const body = await c.req.json();
    const email = body.email;
